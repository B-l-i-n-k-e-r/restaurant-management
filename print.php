...
<head>
    ...
    <style>
        ...
        .watermark {
            ...
            font-size: 40px;
            font-weight: bold;
            color: rgba(78, 205, 196, 0.1); /* Subtle Teal Watermark */
            white-space: nowrap;
            pointer-events: none;
            z-index: 1;
        }
        ...
        @media print {
            ...
            .watermark { color: rgba(0,0,0,0.05); } /* Grayscale for print */
        }
    </style>
</head>
...
<div class="receipt">
    ...
    <div class="header">
        <?php if(!empty($restaurant['restaurant_logo'])): ?>
            <img src="<?php echo $restaurant['restaurant_logo']; ?>">
        <?php endif; ?>
    ...
    <div class="totals-section">
        <div>
            <span>SUBTOTAL</span>
            <span><?php echo $object->cur.number_format($row["order_gross_amount"],2); ?></span>
        </div>

        <?php
        $object->query = "SELECT * FROM order_tax_table WHERE order_id = :order_id";
        $object->execute(['order_id' => $row["order_id"]]);
        foreach($object->statement_result() as $tax):
        ?>
        <div style="font-size: 12px; color: #7f8c8d;">
            <span><?php echo $tax["order_tax_name"]; ?> (<?php echo $tax["order_tax_percentage"]; ?>%)</span>
            <span><?php echo $object->cur.number_format($tax["order_tax_amount"],2); ?></span>
        </div>
        <?php endforeach; ?>
    ...
    <div class="qr">
        ...
        <div style="font-size: 10px; margin-top: 5px;">Scan to Verify Order</div>
    </div>
